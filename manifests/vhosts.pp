...
define nginx::vhosts (
  ...
  file { "${nginx::vhosts_dir}/${subdomain}.conf":
    ...
    group   => $nginx::vhosts_group,
    mode    => '0644',
    content => epp('nginx/vhosts.conf.epp', {'port' => $port, 'subdomain' => $subdomain, 'httpsport' => $httpsport, 'docroot' => $docroot, 'ssl_certificate' => $ssl_certificate, 'ssl_certificate_key' => $ssl_certificate, 'ssl_dhparam' => $ssl_certificate}),
